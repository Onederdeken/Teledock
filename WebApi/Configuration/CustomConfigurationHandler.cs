...
using Teledock.Infrastructure.dbContext.Interceptors;
using Teledock.Infrastructure.Repositories;
    ...
    public static  class CustomConfigurationHandler
    {
        ...
        public static void AddMyDependencyGroup(this IServiceCollection Services, IConfiguration Configuration)
        {
            ...
            Services.AddDbContext<DbCommand>(Options =>
                {
                    ...
                    Options.UseMySql(Configuration.GetConnectionString("DbCommand"), ServerVersion.AutoDetect(Configuration.GetConnectionString("DbCommand")), m => m.MigrationsAssembly("Teledock.Infrastructure"));
            ...
            Services.AddDbContext<DbQuery>(Options => {
                Options.UseMySql(Configuration.GetConnectionString("DbQuery"), ServerVersion.AutoDetect(Configuration.GetConnectionString("DbQuery")), m => m.MigrationsAssembly("Teledock.Infrastructure"));
            });

            //подключаю свой кастомный мапер
